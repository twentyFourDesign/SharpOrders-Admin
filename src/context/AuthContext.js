...
// Initialize Firebase only if on native
if (Platform.OS !== 'web') {
  ...
  // Web initialization inside AuthContext to ensure order
  const firebaseConfig = {
    apiKey: process.env.EXPO_PUBLIC_FIREBASE_API_KEY,
    authDomain: process.env.EXPO_PUBLIC_FIREBASE_AUTH_DOMAIN,
    projectId: process.env.EXPO_PUBLIC_FIREBASE_PROJECT_ID,
  ...
  if (!firebase.apps || !firebase.apps.length) {
    firebase.initializeApp(firebaseConfig);
    console.log('[AuthContext] Firebase initialized for web');
  }
}

// Use a getter to avoid top-level execution errors on web
...
export const AuthProvider = ({children}) => {
  // Initialize auth and db within the provider or as needed
