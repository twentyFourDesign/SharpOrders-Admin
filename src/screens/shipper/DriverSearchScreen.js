...
const DriverSearchScreen = ({navigation, route}) => {
  ...
  const [paymentDriver, setPaymentDriver] = useState(null);

  const handlePaymentSuccess = async response => {
    setShowPayment(false);
    const driver = paymentDriver;
    if (!driver || !loadId) return;

    try {
      // Accept the bid using the new system
      await firebaseLoadsService.acceptBid(driver.bidId, loadId, driver.id);

      // Create shipment (adding this here to match LoadBoardScreen logic)
      const shipmentData = {
        loadId: loadId,
        shipperId: user.uid,
        driverId: driver.id,
        pickupAddress: currentLoad?.pickupAddress || 'Not specified',
        deliveryAddress: currentLoad?.deliveryAddress || 'Not specified',
        truckType: currentLoad?.truckType || 'Standard',
        fareOffer: driver.bidAmount || 0,
        status: 'pending',
        driverAccepted: true,
        shipperAccepted: true,
        acceptedAt: new Date(),
        acceptedBidId: driver.bidId,
        paymentRef: response.reference || response.transaction || 'N/A',
        paymentStatus: 'paid',
      };
      ...
      Alert.alert(
        'Success',
        `Payment successful! Bid from ${driver.name} accepted.`,
        [
          {
            text: 'OK',
            onPress: () =>
      ...
      Alert.alert('Error', 'Failed to complete bid acceptance.');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.driversRow}>
          ...
          <View style={styles.driverAvatars}>
            {interestedDrivers.slice(0, 4).map((driver, index) => (
              <View
                key={`${driver.bidId || driver.id}-${index}`}
                style={styles.avatar}>
      ...
      <PaystackPayment
        ...
        amount={Number(paymentDriver?.price || 0)}
        email={user?.email || ''}
        billingName={user?.displayName || 'Shipper'}
