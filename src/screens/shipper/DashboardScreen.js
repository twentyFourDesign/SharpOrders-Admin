...
const BookingSearch = ({
  ...
  return (
    <View>
      ...
      <TouchableOpacity style={styles.postLoadButton} onPress={onPostNewLoad}>
        <Text style={styles.postLoadButtonText}>Post New Load</Text>
      </TouchableOpacity>
    </View>
  );
};
...
const DashboardScreen = ({navigation}) => {
  const {signOut, user, userType, setUserType} = useAuth();
  ...
  const handleSuggestionPress = item => {
    setQuery(item);
    // searchLocation(); // Removed auto-search
  };
  ...
  useEffect(() => {
    // Reset step to 0 when dashboard is focused or mounted
  ...
  const searchLocation = async () => {
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        // Populate pickup address automatically if empty
        setFormData(prev => ({
          ...prev,
