...
const LoadBoardScreen = ({navigation}) => {
  ...
  const handleAcceptBid = async bid => {
    try {
      ...
      console.log('Initiating payment for bid:', bid.id);
      setPaymentBid(bid);
      setShowPayment(true);
    } catch (error) {
      console.error('Error initiating bid acceptance:', error);
      Alert.alert(
        'Error',
        'Failed to initiate bid acceptance. Please try again.',
      );
    }
  };
  ...
  const handlePaymentSuccess = async response => {
    ...
    const bid = paymentBid;
    if (!bid || !selectedLoad) return;

    try {
      console.log(
        'Payment successful, accepting bid:',
        bid.id,
        'for load:',
        selectedLoad.id,
        bid.driverId,
      ...
      const shipmentData = {
        ...
        paymentRef: response.reference || response.transaction || 'N/A', // Store payment reference
        paymentStatus: 'paid',
      };
      ...
      Alert.alert(
        'Success',
        `Payment successful and bid accepted! Driver will be notified and shipment created.`,
        [
          {
            text: 'OK',
            onPress: () => {
      ...
      console.error('Error accepting bid after payment:', error);
      Alert.alert(
        'Error',
        'Failed to complete bid acceptance. Please contact support.',
      );
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <PaystackPayment
        ...
        amount={paymentBid?.offerAmount || 0}
        email={user?.email || ''}
